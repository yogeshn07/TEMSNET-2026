<!-- ============================================================
     IEEE REFINED MANUSCRIPT — TEMSMET 2026
     Refinement pass: 2026-07-09
     Source: docs/paper/final_submission_manuscript.md
     Reviewer: Senior IEEE Transactions reviewer pass
     Scientific content: LOCKED (no numerical values changed)
     Changes: Writing quality, logical flow, methodology justifications,
              discussion depth, statistical language, IEEE compliance.
              See docs/paper/refinement_changelog.md for full change log.
     LaTeX target: IEEEtran.cls (conference, two-column, A4)
     ============================================================ -->
...
with explanatory sensitivity substantially exceeding any predictive effect across
complementary effect-size measures. These results indicate that accuracy-only
validation is insufficient to characterise explanation stability following NIDS
retraining, and that monitoring explanation consistency is an important component
of responsible model assessment in XAI-augmented NIDS pipelines.
...
strong performance on curated benchmark datasets [10, 13, 16]. A persistent
...
packets [4, 19]. This distributional asymmetry biases standard classifiers toward
majority-class predictions and suppresses detection of rare, high-severity attack
types.
...
to understand which network features drive individual classification decisions. As
AI systems become subject to regulatory transparency requirements [23], the
reliability of these explanations in security-critical applications warrants
...
cost-sensitive learning [19] and ensemble methods [19] have been proposed. SMOTE
remains a dominant approach in NIDS literature due to its simplicity,
reproducibility, and consistent minority-class recall improvements [13, 18].
UNSW-NB15 [4], created at the University of New South Wales Cyber Range Lab,
provides a representative benchmark for studying imbalance in NIDS evaluation:
its predefined training/test split and ten-class taxonomy, with maximum imbalance
ratios exceeding 430:1, make it a standard reproducible evaluation protocol [10, 16].
However, whether rebalancing-driven model changes propagate into XAI explanation
instability — beyond their effect on prediction accuracy — has not been addressed.
...
Forest–based NIDS [20, 21] as well as deep learning–based NIDS [17]. Despite their
adoption, the stability of SHAP and LIME explanations under training data
composition changes — as distinct from robustness to input perturbation — has not
been systematically studied in NIDS contexts.
...
work has examined the sensitivity of explanations to hyperparameter choices [11]
and the impact of data resampling on feature importance in non-NIDS imbalanced
datasets [9, 14]. The specific question of how training data rebalancing affects
SHAP and LIME explanation stability in NIDS — distinct from prediction stability —
has not been addressed in prior literature to the best of the authors' knowledge.
This gap is consequential: if explanation rankings shift substantially when training
distribution changes, practitioners relying on XAI to understand model decisions
may encounter inconsistent attributions after routine retraining events, without
any accuracy signal to alert them. This study addresses this gap directly.
...
imbalance ratio of 430.77:1 (Fig. 1, Fig. 2). UNSW-NB15 was selected because its
predefined split enables reproducible evaluation without researcher-induced
variation, and its extreme imbalance provides a challenging real-world scenario for
studying the interplay between class rebalancing and explanation stability.
...
56,000 each). A total of 384,659 synthetic samples were generated. SMOTE was
selected as the rebalancing technique because it is the most widely applied method
in NIDS literature [10, 13, 18, 19], ensuring that findings are directly relevant
to existing deployments. The test set was not modified; both models are evaluated
on the same 82,332-row held-out set.

### D. Classifier

A Random Forest [1] classifier was trained independently on the original imbalanced
dataset (Baseline model) and on the SMOTE-balanced dataset (SMOTE model). Random
Forest was selected because it achieves competitive performance on UNSW-NB15
[10, 16], supports exact Shapley-value computation via SHAP TreeExplainer without
approximation [7, 8], and is a widely adopted baseline model in NIDS research,
ensuring broad applicability of the findings. All hyperparameters and the global
random seed (42) were held constant between conditions. The original UNSW-NB15
train/test split was used without re-splitting.
...
The 60-instance sample (6 per class) was chosen to ensure equal class
representation across all 10 traffic categories, providing balanced coverage
for global importance aggregation while remaining computationally tractable
for exact SHAP computation over a 100-tree Random Forest.

### F. Statistical Validation

Four hypothesis tests (Research Questions RQ1–RQ4) were conducted (Table IV).
The study predicts that class rebalancing will alter SHAP and LIME attribution
rankings more than aggregate predictive metrics (RQ3).
...
1. **McNemar's test** (Yates continuity correction) on the 82,332-sample paired
   ...
   significant rate; implemented via `scipy.stats.mcnemar` [22]. Discordant pairs:
...
(LIME), power is approximately 0.90. The SHAP Wilcoxon result should accordingly
be interpreted as reaching significance at modest power — a non-significant result
at n = 42 would not have been conclusive evidence of no shift.
...
change −0.020; Normal: 0.848 → 0.809). This trade-off is the characteristic
...
**Summary (RQ1):** SMOTE was partially beneficial — statistically significant
...
**Summary (RQ2):** SMOTE caused a statistically significant change in XAI
...
<!-- FIG. 24 | file: outputs/figures/effect_sizes.png
     ...
     metrics; n = 82,332 paired predictions) and rank-biserial r
     (Wilcoxon tests on feature importances; n = 42 SHAP, n = 36 LIME).
     All predictive effects are negligible (|h| ≤ 0.087); explanatory
     effects range from small to large (r = 0.169–0.595). The two
...
**Summary (RQ3):** Explanatory sensitivity substantially exceeds predictive
...
**TABLE III**
**Effect Sizes: Baseline vs. SMOTE Model**

| Comparison | Effect Metric | *n* | Value | Magnitude |
|:---|:---|---:|---:|:---|
| Accuracy | Cohen's h | 82,332 | −0.0868 | Negligible |
| Macro F1 | Cohen's h | 82,332 | +0.0340 | Negligible |
| Weighted F1 | Cohen's h | 82,332 | −0.0354 | Negligible |
| Confidence score shift | Rank-biserial r | 82,332 | 0.1686 | Small |
| SHAP importance shift | Rank-biserial r | 42 | 0.3259 | Medium |
| LIME importance shift | Rank-biserial r | 36 | 0.5947 | Large |

*Cohen's h = 2 arcsin(√p₁) − 2 arcsin(√p₂) on paired proportion differences.*
*Rank-biserial r = Z / √N from the Wilcoxon normal approximation.*
*Magnitude thresholds: Cohen's h — negligible |h| < 0.20; rank-biserial r —*
*small ≥ 0.10, medium ≥ 0.30, large ≥ 0.50 (Cohen 1988). Rows represent*
*different scales and sample sizes; cross-row comparisons are qualitative.*
...
consistent with the study prediction (§III-F) that class rebalancing alters XAI
explanation structure more than aggregate predictive metrics. LIME's greater
sensitivity relative to SHAP (Δr = 0.269) is consistent with perturbation-based
local explanation methods being more sensitive to training distribution changes than
exact tree-attribution methods.
...
structure — has a precise mechanistic interpretation. Class rebalancing shifts the
Random Forest's decision boundary: minority-class regions, previously
under-represented in training, receive proportionally more support from the 100-tree
ensemble after SMOTE rebalancing. This boundary shift alters the relative importance
weights assigned to features that discriminate minority attack categories from
Normal traffic, even when the aggregate classification accuracy over the full test
set remains essentially unchanged. Post-hoc explanation methods are sensitive to
exactly this kind of local boundary change because they interrogate the model's
local decision surface rather than summarising outputs with a single scalar metric.

The greater sensitivity of LIME relative to SHAP is attributable to their
fundamentally different attribution mechanisms. LIME constructs a local linear
surrogate by sampling perturbed instances in the neighbourhood of each target
instance and fitting a weighted linear model to the RF's responses on those
perturbations; the surrogate therefore directly reflects the updated local decision
boundary. SHAP TreeExplainer, by contrast, computes Shapley values by averaging
contributions across all paths in the tree ensemble; the ensemble-averaging over
100 trees attenuates localised boundary changes, producing a more stable global
attribution ranking. This mechanism explains why LIME's effect size (r = 0.5947)
is substantially larger than SHAP's (r = 0.3259) for the same pair of models.

### B. Practical Implications

These results carry concrete implications for practitioners deploying XAI-augmented
NIDS systems.

Accuracy-only validation is insufficient following any retraining event that changes
training class distribution. When a NIDS model is retrained with a different class
balance (e.g., after periodic SMOTE rebalancing or data collection updates),
explanation outputs may shift substantially even when aggregate accuracy changes
are negligible. Re-validating XAI outputs after retraining should be standard
practice.

SMOTE's predictive trade-off warrants deployment-context analysis. Of 82,332
test instances, SMOTE gained 1,632 (1.98%) predictions but lost 4,784 (5.81%).
For NIDS deployed in environments where detecting rare, high-severity attacks is
the primary objective, the minority-class F1 improvement (e.g., Worms: 0.233 →
0.452) justifies this trade-off. For environments where minimising false positives
is critical, it warrants careful evaluation.

LIME-based explanations require additional scrutiny after distribution changes.
LIME's large sensitivity (r = 0.595) means that a change in training class
distribution can produce substantially different attribution rankings without a
corresponding accuracy signal. This sensitivity is partly structural — LIME's
perturbation-based mechanism is inherently more responsive to local boundary
changes than ensemble-level Shapley values — and is compounded by the moderate
local surrogate fidelity (R² ≈ 0.25–0.29) observed in this study.

Explanation-consistency monitoring should be incorporated into XAI pipelines
alongside accuracy monitoring. Tracking Spearman rank correlation of feature
importance rankings before and after retraining provides a lightweight signal for
explanation drift detection that accuracy-only monitoring cannot provide.
...
transparency requires that explanations be provided for consequential automated
decisions [23]. If those explanations are unstable under routine model updates —
such as class rebalancing — the explanations themselves become unreliable even when
performance metrics remain acceptable. To the best of the authors' knowledge, this
study provides the first empirical evidence of disproportionate explanation
instability in a standardised NIDS benchmark setting with rigorous paired
...
- **LIME surrogate fidelity:** Mean local R² was 0.290 (Baseline) and 0.252
  ...
  (r = 0.5947) is statistically robust, but absolute weights should not be
...
- **Synthetic minority samples:** SMOTE generated 384,659 synthetic instances.
  Synthetic samples may not faithfully represent the statistical properties of real
  minority-class attack traffic in production networks.
...
- **Operationalisation of explanation quality:** Explanation quality is
  operationalised as feature rank stability (Spearman correlation, top-k overlap)
  and importance magnitude shift (Wilcoxon). Human interpretability and
  ground-truth fidelity are not evaluated; rank stability is a necessary but
  not sufficient condition for explanation quality.
...
- **Holm–Bonferroni family-wise correction** controls the family-wise error rate
  across four tests but does not account for any exploratory analyses performed
  during feature engineering or model selection. The four formal tests correspond
  directly to the four pre-specified research questions.
- **Normal approximation for Wilcoxon** at n = 82,332 (confidence scores) is
  justified by the large sample size but may be conservative for the small-n tests
  (n = 42, 36). The exact Wilcoxon permutation test was not feasible at n = 82,332;
  at n = 42 and n = 36 the normal approximation is adequate for the observed effect
  sizes.
...
Several natural extensions follow from the limitations identified above.

Alternative rebalancing techniques — ADASYN, random undersampling, class-weighted
loss, and ensemble-based methods — may produce qualitatively different explanation
shifts. A systematic comparison across rebalancing strategies would generalise the
finding that rebalancing changes explanation structure more than prediction accuracy.

The analysis should be extended across model families. Gradient-based SHAP for
neural NIDS models and Integrated Gradients may exhibit different sensitivity
profiles to class rebalancing; characterising these differences would inform XAI
tool selection across the model space.

Multi-dataset validation on CICIDS-2017, NSL-KDD, or contemporary operational
enterprise logs would strengthen external validity. Of particular value would be
datasets captured after 2020 to address the temporal scope limitation of UNSW-NB15.

Human-centred evaluation is needed to determine whether the observed rank shifts
constitute meaningful changes from a security analyst's perspective. User studies
measuring analyst decision quality under Baseline vs. SMOTE explanations would
connect the statistical findings to operational impact.

Formal explanation-consistency metrics for NIDS model certification represent a
practical research need, particularly as regulatory frameworks [23] begin to
require auditability of consequential AI decisions in critical infrastructure.

---

## IX. Conclusion

SMOTE-based class rebalancing produces statistically significant but practically
negligible changes to aggregate predictive metrics (max Cohen's h = 0.087) while
producing medium-to-large changes to SHAP and LIME feature attribution rankings
(rank-biserial r up to 0.595 for LIME) in a Random Forest NIDS evaluated on the
UNSW-NB15 benchmark. The primary finding — that LIME explanation sensitivity to
class rebalancing substantially and disproportionately exceeds predictive
sensitivity — demonstrates that accuracy-only validation is insufficient for
XAI-augmented NIDS pipelines. Practitioners who rely solely on accuracy or F1
to validate a retrained model may unknowingly deploy models whose explanation
attribution rankings have changed substantially, potentially misleading security
analysts who rely on those explanations for decision support. This finding holds
even under moderate LIME local surrogate fidelity (R² ≈ 0.25–0.29): the
statistical evidence for a large explanatory shift is robust regardless of absolute
attribution magnitudes.

The secondary finding — that LIME is substantially more sensitive than SHAP to
training distribution changes — has methodological implications for XAI tool
selection in security applications: perturbation-based local explanation methods
should be independently validated after any retraining event that alters the
training class distribution, and local surrogate fidelity should be reported
alongside attribution rankings.

Future work should validate these findings across additional datasets, model
families, and rebalancing strategies, and develop formal explanation-consistency
metrics to support responsible deployment of XAI-augmented NIDS in regulatory
environments.
...
Commun. Technol. (ICICT)*, Feb. 2020, pp. 41–45.
...
<!-- ============================================================
     ...
     Fig. 19 — outputs/figures/explanation_ranking_comparison.png → §V-B, full-width KEY
     ...
     MINIMUM FIGURE SET (6-page target):
